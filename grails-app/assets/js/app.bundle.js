				...
				switch ( true ) {
					...
					case ( actiontype === 'panel-close' ):
						...
						if (typeof bootbox  != 'undefined') {
							...
							bootbox.confirm({
								title: "<i class='fal fa-times-circle text-danger mr-2'></i> Estás seguro que quieres borrar el panel <span class='fw-500'>&nbsp;'" +selectedPanel.children('.panel-hdr').children('h2').text().trim()+ "'&nbsp;</span>?",
							...
							if (confirm( 'Estás seguro que quieres borrar el panel ' + selectedPanel.children('.panel-hdr').children('h2').text().trim() + '?' )) {
								killPanel();
							}
					...
					case ( actiontype === 'theme-update' ):
						...
						if ( $(myapp_config.mythemeAnchor).length) {
							...
							// var mytheme = $("<link>", {id: myapp_config.mythemeAnchor.replace('#', '','confEntidad'), "rel": "stylesheet", "href" : "test" });
							$('head').append(mytheme);
						}
